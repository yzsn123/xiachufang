...
export default {
  ...
  methods: {
    ...
    async checkLogin() {
      //检查是否登录
      await this.$store.dispatch("requestCheckLogin");
    }
  },
  created() {
    ...
    this.$refs.MarkSrc.ontouchstart = ev => {
      ...
      this.$refs.MarkSrc.ontouchmove = ev => {
        ...
        if (ChangeMarkHeight > MarkHeight) {
          ...
          let rate = (ChangeMarkHeight - MarkHeight) / BodyHeight;
          this.$refs.mark.style.opacity = 1 - rate;
